...
func target_selected_card(target_instance_id: String) -> Dictionary:
	...
	if not _can_resolve_selected_action(selected_card):
		return _invalid_ui_result(_screen._status_message)
	var target_location := MatchMutations.find_card_location(_screen._match_state, target_instance_id)
	...
	var selected_location := MatchMutations.find_card_location(_screen._match_state, _screen._selected_instance_id)
	var saved_item_id := ""
	var saved_action_id := ""
	...
	if bool(selected_location.get("is_valid", false)) and str(selected_location.get("zone", "")) == MatchMutations.ZONE_HAND and str(selected_card.get("card_type", "")) == "item":
		...
		if _is_pending_prophecy_card(selected_card):
			result = MatchTiming.play_pending_prophecy(_screen._match_state, str(selected_card.get("controller_player_id", "")), _screen._selected_instance_id, {"target_instance_id": target_instance_id})
		else:
			result = PersistentCardRules.play_item_from_hand(_screen._match_state, _active_player_id(), _screen._selected_instance_id, {"target_instance_id": target_instance_id})
		...
		if _is_pending_prophecy_card(selected_card):
			...
			result = MatchTiming.play_action_from_hand(_screen._match_state, _active_player_id(), _screen._selected_instance_id, {"target_instance_id": target_instance_id})
	elif bool(selected_location.get("is_valid", false)) and str(selected_location.get("zone", "")) == MatchMutations.ZONE_SUPPORT:
		result = PersistentCardRules.activate_support(_screen._match_state, _active_player_id(), _screen._selected_instance_id, {"target_instance_id": target_instance_id})
	elif bool(selected_location.get("is_valid", false)) and str(selected_location.get("zone", "")) == MatchMutations.ZONE_LANE:
		result = MatchCombat.resolve_attack(_screen._match_state, _active_player_id(), _screen._selected_instance_id, {"type": "creature", "instance_id": target_instance_id})
	else:
...
func attack_selected_player(player_id: String) -> Dictionary:
	...
	if not _can_resolve_selected_action(selected_card):
		return _invalid_ui_result(_screen._status_message)
	var result := MatchCombat.resolve_attack(_screen._match_state, _active_player_id(), _screen._selected_instance_id, {"type": "player", "player_id": player_id})
...
func _selected_support_uses_card_targets(card: Dictionary) -> bool:
	if card.is_empty() or _selected_action_mode(card) != SELECTION_MODE_SUPPORT or _is_pending_prophecy_card(card):
		return false
	var location := MatchMutations.find_card_location(_screen._match_state, str(card.get("instance_id", "")))
...
func _play_action_to_lane(lane_id: String) -> void:
	...
	if _is_pending_prophecy_card(card):
		result = MatchTiming.play_pending_prophecy(_screen._match_state, str(card.get("controller_player_id", "")), _screen._selected_instance_id, options)
	else:
		result = MatchTiming.play_action_from_hand(_screen._match_state, _active_player_id(), _screen._selected_instance_id, options)
...
func _play_mobilize_item_to_lane(lane_id: String) -> void:
	...
	var result := PersistentCardRules.play_item_from_hand(_screen._match_state, _active_player_id(), saved_instance_id, {"lane_id": lane_id})
...
func _check_summon_target_mode(source_instance_id: String) -> void:
	...
	abilities = abilities.filter(func(ab):
		if bool(ab.get("consume", false)):
			return false
		if not MatchTiming._summon_ability_conditions_met(_screen._match_state, card, ab):
			return false
	...
	if abilities.is_empty():
		return
	var valid_targets := MatchTiming.get_all_valid_targets(_screen._match_state, source_instance_id)
...
func _resolve_summon_target_card(target_instance_id: String) -> void:
	...
	var valid_targets := MatchTiming.get_all_valid_targets(_screen._match_state, source_id)
	var is_valid := false
	...
	if is_turn_trigger:
		var result := MatchTiming.resolve_pending_turn_trigger_target(_screen._match_state, _local_player_id(), {"instance_id": target_instance_id})
		...
		var result := MatchTiming.resolve_pending_summon_effect_target(_screen._match_state, _local_player_id(), {"target_instance_id": target_instance_id})
		_finalize_engine_result(result, "Targeted %s." % _card_name(_card_from_instance_id(target_instance_id)))
		_check_deferred_betray()
	else:
		var result := MatchTiming.resolve_targeted_effect(_screen._match_state, source_id, {"target_instance_id": target_instance_id})
		_finalize_engine_result(result, "Targeted %s." % _card_name(_card_from_instance_id(target_instance_id)))
		_check_deferred_betray()
...
func _resolve_summon_target_player(player_id: String) -> void:
	...
	_cancel_targeting_mode_silent()
	if is_turn_trigger:
		var result := MatchTiming.resolve_pending_turn_trigger_target(_screen._match_state, _local_player_id(), {"player_id": player_id})
		_finalize_engine_result(result, "Targeted %s." % _player_name(player_id))
		_check_pending_turn_trigger_target()
	elif is_effect_summon:
		var result := MatchTiming.resolve_pending_summon_effect_target(_screen._match_state, _local_player_id(), {"target_player_id": player_id})
		_finalize_engine_result(result, "Targeted %s." % _player_name(player_id))
		_check_deferred_betray()
	else:
		var result := MatchTiming.resolve_targeted_effect(_screen._match_state, source_id, {"target_player_id": player_id})
		_finalize_engine_result(result, "Targeted %s." % _player_name(player_id))
		_check_deferred_betray()
...
func _is_pending_summon_mandatory() -> bool:
	...
	if bool(_pending_summon_target.get("is_effect_summon", false)):
		var pending := MatchTiming.get_pending_summon_effect_target(_screen._match_state, _local_player_id())
...
func _cancel_summon_target_mode() -> void:
	var is_turn_trigger := bool(_pending_summon_target.get("is_turn_trigger", false))
	if is_turn_trigger:
		MatchTiming.decline_pending_turn_trigger_target(_screen._match_state, _local_player_id())
	elif bool(_pending_summon_target.get("is_effect_summon", false)):
		MatchTiming.decline_pending_summon_effect_target(_screen._match_state, _local_player_id())
	_pending_summon_target = {}
	_dismiss_summon_skip_button()
	_cancel_targeting_mode()
...
func _check_pending_secondary_target() -> void:
	...
	if not MatchTiming.has_pending_secondary_target(_screen._match_state, local_id):
		return
	if not _pending_secondary_target_state.is_empty():
		return
	if not _pending_summon_target.is_empty():
		return
	var pending := MatchTiming.get_pending_secondary_target(_screen._match_state, local_id)
...
func _resolve_secondary_target_card(target_instance_id: String) -> void:
	...
	_cancel_targeting_mode_silent()
	var result := MatchTiming.resolve_pending_secondary_target(_screen._match_state, _local_player_id(), target_instance_id)
...
func _resolve_secondary_target_player(player_id: String) -> void:
	_pending_secondary_target_state = {}
	_cancel_targeting_mode_silent()
	var result := MatchTiming.resolve_pending_secondary_target_player(_screen._match_state, _local_player_id(), player_id)
	_finalize_engine_result(result, "Dealt damage to %s." % _player_name(player_id))


func _check_pending_summon_effect_target() -> void:
	var local_id := _local_player_id()
	if not MatchTiming.has_pending_summon_effect_target(_screen._match_state, local_id):
		return
	if not _pending_summon_target.is_empty():
		return
	var pending := MatchTiming.get_pending_summon_effect_target(_screen._match_state, local_id)
	var source_id := str(pending.get("source_instance_id", ""))
	var card := _card_from_instance_id(source_id)
	if card.is_empty():
		MatchTiming.decline_pending_summon_effect_target(_screen._match_state, local_id)
		return
	var valid_targets := MatchTiming.get_all_valid_targets(_screen._match_state, source_id)
	if valid_targets.is_empty():
		MatchTiming.decline_pending_summon_effect_target(_screen._match_state, local_id)
...
func _show_summon_skip_button() -> void:
	...
	_screen._summon_skip_button = Button.new()
	_screen._summon_skip_button.text = "Skip"
	_screen._summon_skip_button.custom_minimum_size = Vector2(120, 44)
	...
	_screen._summon_skip_button.add_theme_stylebox_override("normal", style)
	_screen._summon_skip_button.add_theme_stylebox_override("hover", style)
	_screen._summon_skip_button.add_theme_stylebox_override("pressed", style)
	_screen._summon_skip_button.add_theme_color_override("font_color", Color(0.9, 0.88, 0.92, 1.0))
	_screen._summon_skip_button.add_theme_font_size_override("font_size", 16)
	_screen._summon_skip_button.pressed.connect(_cancel_summon_target_mode)
	_screen._summon_skip_button.z_index = 600
	add_child(_screen._summon_skip_button)
	_screen._summon_skip_button.position = Vector2(viewport_size.x * 0.5 - 60, viewport_size.y * 0.5 + 40)


func _dismiss_summon_skip_button() -> void:
	if _screen._summon_skip_button != null and is_instance_valid(_screen._summon_skip_button):
		_screen._summon_skip_button.queue_free()
	_screen._summon_skip_button = null
