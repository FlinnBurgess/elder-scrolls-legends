	...
	match op:
		...
		"equip_copy_of_item":
			...
			if ecoi_item.is_empty():
				return
			# A copy generated by this op isn't itself "copyable" — prevents infinite
			# loops when multiple Gardeners of Swords are in play.
			if bool(ecoi_item.get("_is_equip_copy", false)):
				return
