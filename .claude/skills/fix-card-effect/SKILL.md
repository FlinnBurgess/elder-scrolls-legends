...
9. **Inline `card_template` missing abilities** — Two sub-cases:
   ...
   - `build_generated_card` catalog lookup copies: `triggered_abilities`, `aura`, `innate_statuses`, `first_turn_hand_cost`, `first_turn_hand_magicka`. It does **not** copy `grants_immunity`, `cannot_attack`, or other fields — these must be added to the template manually if needed.
