...
- **Generalized fixes can introduce regressions.** When a pattern fix involves broadening a condition (e.g., removing a whitelist filter), the broader change may affect code paths beyond the target bugs. Always run the full relevant test suite after applying a pattern fix, and if tests fail, narrow the fix to only the intended families/cases rather than a blanket change.
